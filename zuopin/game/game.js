...
game.prototype={
    ...
    move2:function(){
        for (var i = 0; i < that.span.length; i++) {
            ...
            if(tops>that.height){
                ...
                that.randArr.splice(i,1);
                that.createEle(1);
                var die=document.getElementById("shengming");
    ...
    createEle:function(num){
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            span.style.cssText="position:absolute;height:80px;width:80px;font-size:30px;font-family:Arial;color:rgba(255,255,255,0);top:"+(-300*Math.random()-85)+"px;left:"+(50+Math.random()*(this.width-200))+"px;background: url(image/"+arr[i]+".png) no-repeat center center;background-size:80px 80px;";
